...
void Maze::Solve(const CellMatrixCoordinates& start_cell,
  ...
  while (true) {
    s21::MazeCell current_cell = solving_stack_.top();
    ...
    if (solving_stack_.empty()) {
      throw s21::MazeSolutionDoesNotExists();
    }
  }
...
void Maze::CreateSolvingPath() {
  while (!solving_stack_.empty()) {
    s21::MazeCell top_cell = solving_stack_.top();
...
bool Maze::IsInithialized() const {
  return cells_.GetRows() != s21::kDefaultRows;
}
