...
MainWindow::MainWindow(QWidget* parent)
  ...
  const s21::Maze* maze = controller_.GetMazeReference();
  ui->GLwidget->SetMaze(maze);

  const s21::Cave* cave = controller_.GetCaveReference();
...
void MainWindow::on_solve_maze_clicked() {
  ...
  } catch (s21::InvalidCellCoordinates&) {
    ShowMessageBox("Некорректные координаты ячеек");
  } catch (s21::MazeSolutionDoesNotExists&) {
