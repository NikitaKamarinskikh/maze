...
void CaveWidget::DrawCave() {
  s21::Matrix<s21::CaveCell> cells = cave_->GetCells();
  glLineWidth(2);
  glPointSize(10);
  glBegin(GL_QUADS);
  glColor3f(0, 0, 0);
  for (s21::Matrix<s21::CaveCell>::size_type i = 0; i < cells.GetRows(); ++i) {
    for (s21::Matrix<s21::CaveCell>::size_type j = 0; j < cells.GetColumns();
         ++j) {
...
void CaveWidget::DrawCell(const s21::CaveCell& cell) {
...
void CaveWidget::SetCave(const s21::Cave* cave) { cave_ = cave; }
